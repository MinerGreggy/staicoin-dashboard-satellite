...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'staicoin-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('staicoin').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('staicoin')[service]);
